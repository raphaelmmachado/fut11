import { Position } from "../types/typing";

const fourFourTwo: Position = [
  //gol
  { top: "2%", left: "50%", transform: "translateX(-50%)" },
  //ld
  { top: "20%", left: "15%", transform: "translateX(-50%)" },
  //z
  { top: "15%", left: "35%", transform: "translateX(-50%)" },
  { top: "15%", left: "65%", transform: "translateX(-50%)" },
  //md
  { top: "40%", left: "35%", transform: "translateX(-50%)" },
  //le
  { top: "20%", left: "85%", transform: "translateX(-50%)" },
  //ma
  { top: "55%", left: "25%", transform: "translateX(-50%)" },
  //md
  { top: "40%", left: "65%", transform: "translateX(-50%)" },
  //atk
  { top: "75%", left: "35%", transform: "translateX(-50%)" },
  //ma
  { top: "55%", left: "75%", transform: "translateX(-50%)" },
  { top: "75%", left: "65%", transform: "translateX(-50%)" },
];
const fourFourTwoLosangle: Position = [
  //gol
  { top: "2%", left: "50%", transform: "translateX(-50%)" },
  //ld
  { top: "20%", left: "15%", transform: "translateX(-50%)" },
  //z
  { top: "15%", left: "35%", transform: "translateX(-50%)" },
  { top: "15%", left: "65%", transform: "translateX(-50%)" },
  //md
  { top: "35%", left: "50%", transform: "translateX(-50%)" },
  //le
  { top: "20%", left: "85%", transform: "translateX(-50%)" },
  //mc
  { top: "45%", left: "75%", transform: "translateX(-50%)" },
  { top: "45%", left: "25%", transform: "translateX(-50%)" },
  //atk
  { top: "75%", left: "35%", transform: "translateX(-50%)" },
  //ma
  { top: "55%", left: "50%", transform: "translateX(-50%)" },
  //atk
  { top: "75%", left: "65%", transform: "translateX(-50%)" },
];
const fourThreeThree: Position = [
  { top: "2%", left: "50%", transform: "translateX(-50%)" },
  { top: "20%", left: "15%", transform: "translateX(-50%)" },
  { top: "15%", left: "35%", transform: "translateX(-50%)" },
  { top: "15%", left: "65%", transform: "translateX(-50%)" },
  { top: "40%", left: "50%", transform: "translateX(-50%)" },
  { top: "20%", left: "85%", transform: "translateX(-50%)" },
  { top: "45%", left: "75%", transform: "translateX(-50%)" },
  { top: "45%", left: "25%", transform: "translateX(-50%)" },
  { top: "75%", left: "25%", transform: "translateX(-50%)" },
  { top: "70%", left: "50%", transform: "translateX(-50%)" },
  { top: "75%", left: "75%", transform: "translateX(-50%)" },
];
const fourThreeThreeB: Position = [
  { top: "2%", left: "50%", transform: "translateX(-50%)" },
  { top: "20%", left: "15%", transform: "translateX(-50%)" },
  { top: "15%", left: "35%", transform: "translateX(-50%)" },
  { top: "15%", left: "65%", transform: "translateX(-50%)" },
  { top: "40%", left: "50%", transform: "translateX(-50%)" },
  { top: "20%", left: "85%", transform: "translateX(-50%)" },
  { top: "45%", left: "75%", transform: "translateX(-50%)" },
  { top: "45%", left: "25%", transform: "translateX(-50%)" },
  { top: "75%", left: "50%", transform: "translateX(-50%)" },
  { top: "70%", left: "25%", transform: "translateX(-50%)" },
  { top: "70%", left: "75%", transform: "translateX(-50%)" },
];
const threeFiveTwo: Position = [
  //gol
  { top: "2%", left: "50%", transform: "translateX(-50%)" },
  //ala direito
  { top: "40%", left: "15%", transform: "translateX(-50%)" },
  //z dir
  { top: "20%", left: "25%", transform: "translateX(-50%)" },
  // z esq
  { top: "20%", left: "75%", transform: "translateX(-50%)" },
  //z central
  { top: "20%", left: "50%", transform: "translateX(-50%)" },
  //ala esquerda
  { top: "40%", left: "85%", transform: "translateX(-50%)" },

  // meias ofe
  { top: "55%", left: "70%", transform: "translateX(-50%)" },
  // meia def
  { top: "45%", left: "50%", transform: "translateX(-50%)" },

  // atk
  { top: "75%", left: "35%", transform: "translateX(-50%)" },
  { top: "55%", left: "30%", transform: "translateX(-50%)" },
  { top: "75%", left: "65%", transform: "translateX(-50%)" },
];
const threeFourThree: Position = [
  //gol
  { top: "2%", left: "50%", transform: "translateX(-50%)" },
  //ala direito
  { top: "50%", left: "15%", transform: "translateX(-50%)" },
  //z dir
  { top: "20%", left: "25%", transform: "translateX(-50%)" },
  // z esq
  { top: "20%", left: "75%", transform: "translateX(-50%)" },
  //z central
  { top: "20%", left: "50%", transform: "translateX(-50%)" },
  //ala esquerda
  { top: "50%", left: "85%", transform: "translateX(-50%)" },

  // meias ofe
  { top: "40%", left: "65%", transform: "translateX(-50%)" },
  // meia def
  { top: "40%", left: "35%", transform: "translateX(-50%)" },

  // atk
  { top: "70%", left: "25%", transform: "translateX(-50%)" },
  { top: "75%", left: "50%", transform: "translateX(-50%)" },
  { top: "70%", left: "75%", transform: "translateX(-50%)" },
];
const fiveFourOne: Position = [
  //gol
  { top: "2%", left: "50%", transform: "translateX(-50%)" },
  //ala direito
  { top: "35%", left: "15%", transform: "translateX(-50%)" },
  //z dir
  { top: "20%", left: "25%", transform: "translateX(-50%)" },
  // z esq
  { top: "20%", left: "75%", transform: "translateX(-50%)" },
  //z central
  { top: "20%", left: "50%", transform: "translateX(-50%)" },
  //ala esquerda
  { top: "35%", left: "85%", transform: "translateX(-50%)" },

  // meias ofe
  { top: "50%", left: "65%", transform: "translateX(-50%)" },
  // meia def
  { top: "50%", left: "35%", transform: "translateX(-50%)" },

  // atk
  { top: "75%", left: "50%", transform: "translateX(-50%)" },
  { top: "65%", left: "20%", transform: "translateX(-50%)" },
  { top: "65%", left: "80%", transform: "translateX(-50%)" },
];
export {
  fourFourTwo,
  fourFourTwoLosangle,
  fourThreeThree,
  fourThreeThreeB,
  threeFiveTwo,
  threeFourThree,
  fiveFourOne,
};
